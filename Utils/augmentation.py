...
class ExtToTensor(object):
    ...
    def __call__(self, pic, lbl):
        ...
        """
        if self.normalize:
            return F.to_tensor(pic), torch.from_numpy(np.array(lbl, dtype=self.target_type).transpose((2, 0, 1)))
        else:
            return torch.from_numpy(np.array(pic, dtype=np.float32).transpose((2, 0, 1))),\
                   torch.from_numpy(np.array(lbl, dtype=self.target_type).transpose((2, 0, 1)))
...
class ExtRandomResizedCrop(object):
