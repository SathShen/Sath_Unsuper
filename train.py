...
def train(frame, cfgs):
    ...
    valid_dataset = LocalDatasetBuilder(cfgs, cfgs.DATA.VALID_DATA_PATH)
    ...
    valid_data_loader = torch.utils.data.DataLoader(valid_dataset, batch_size=cfgs.DATA.BATCH_SIZE,
                                                     shuffle=True, num_workers=cfgs.DATA.NUM_WORKERS)

    logger = Logger(cfgs.NET.NAME, cfgs.CFG_NOTE)
    logger.log_in(f'Start on device_ids: {frame.device_ids}!', f'{train_dataset.__len__()} examples in training set and '
                                               f'{valid_dataset.__len__()} examples in validation set')
    best_loss, best_train_mIoU, best_valid_mIoU = 999, 999, 999
