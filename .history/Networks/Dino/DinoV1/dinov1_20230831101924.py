...
class DinoV1(nn.Module):
    ...
    def forward_one(self, crops_list, backbone, head):
        ...
        for end_idx in idx_crops:
            _out = backbone(torch.cat(crops_list[start_idx: end_idx]))  # cat一样宽度的输入网络
    ...
    def forward(self, crops_list):
        ...
        teacher_output = self.forward_one(crops_list[:2], self.teacher_backbone, self.student_backbone)
