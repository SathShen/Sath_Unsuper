...
def train(frame, cfgs):
    ...
    for epoch in range(cfgs.TRAIN.START_EPOCH, cfgs.TRAIN.NUM_EPOCHS):
        ...
        train_epoch_loss = 0
        for imgs in tqdm(train_data_loader_iter, ncols=80):   # imgs: batch_size, channels, H, W
...
def get_parserargs():
    ...
    parser.add_argument('--learning_rate', '-lr', metavar='LR', type=float, help='Learning rate (max learning rate)')
    parser.add_argument('--lrs_min_value', '-lrsmv', metavar='LRSMV', type=float, help='min value of cosinewarm scheduler')
    parser.add_argument('--lrs_warmup_iters', '-lrswi', metavar='LRSWI', type=int, help='warmup iters of cosinewarm scheduler')
    # start_warmup_value=0, freeze_iters=0, is_restart=False, T_0=5, T_mult=2
    parser.add_argument('--lrs_warmup_value', '-lrswv', metavar='LRSSWV', type=float, help='start warmup value of cosinewarm scheduler')
    parser.add_argument('--lrs_freeze_iters', '-lrsfi', metavar='LRSFI', type=int, help='freeze iters of cosinewarm scheduler')
    parser.add_argument('--lrs_is_restart', '-lrsir', metavar='LRSIR', default=False, help='is restart of cosinewarm scheduler')
    parser.add_argument('--lrs_T_0', '-lrst0', metavar='LRSTMA', type=int, help='T_max of cosinewarm scheduler')
    parser.add_argument('--lrs_T_mult', '-lrstmu', metavar='LRSTMU', type=int, help='T_mult of cosinewarm scheduler')
    parser.add_argument('--lrs_T_max', '-lrstm', metavar='LRSTM', type=int, help='T_max of cosinewarm scheduler')
    ...
    parser.add_argument('--lrs_gamma', '-lrsg', metavar='LRSG', type=float, help='gamma of multistep scheduler')
    parser.add_argument('--lrs_last_epoch', '-lrsl', metavar='LRSL', type=int, help='last_epoch of multistep scheduler')
