...
def train(cfg, frame, train_dataset):
    ...
    train_data_loader = torch.utils.data.DataLoader(train_dataset, 
                                                    ...
                                                    pin_memory=cfg.DATA.IS_PIN_MEMORY,
                                                    drop_last=True
                                                    )
                                                     
    logger = Logger(cfg.NET.NAME, cfg.CFG_NOTE)

    logger.info(f'rank: {global_rank}, world_size: {world_size}, local_rank: {cfg.LOCAL_RANK}',
                f'{train_dataset.__len__()} examples in training set',
                f"number of params: {frame.n_parameters} in model {cfg.NET.NAME}")
    ...
    for epoch in range(cfg.START_EPOCH, cfg.NUM_EPOCHS):
        ...
        if epoch == cfg.START_EPOCH:
            ...
            frame.save_best_weights(cfg.OUTPUT_PATH, cfg.NET.NAME, cfg.CFG_NOTE, epoch, train_epoch_loss)
        else:
            if train_epoch_loss < best_loss:
                best_loss = train_epoch_loss
                frame.save_best_weights(cfg.OUTPUT_PATH, cfg.NET.NAME, cfg.CFG_NOTE, epoch, train_epoch_loss)
        if epoch % cfg.SAVE_FREQ == 0:
            frame.save_weights(cfg.OUTPUT_PATH, cfg.NET.NAME, cfg.CFG_NOTE, epoch)
    ...
    total_timer.stop()
    logger.info(f'train_time: {epoch_timer.get_sumtime()}, '
    ...
    logger.save_log(cfg.OUTPUT_PATH)
...
if __name__ == '__main__':
    ...
    torch.distributed.init_process_group(backend='nccl', init_method='env://', world_size=cfg.WORLD_SIZE, rank=cfg.GLOBAL_RANK)
    torch.distributed.barrier()

    seed = cfg.SEED + cfg.GLOBAL_RANK
