...
class PretrainFrame():
    def __init__(self, cfg, num_dataset):
        ...
        self.num_iter_per_epoch = num_dataset // cfg.DATA.BATCH_SIZE_PER_GPU + 1  # 训练一个batch即一个iter
        ...
        self.optimizer = build_optimizer(cfg, self.net.module.get_params_groups())
    ...
    def optimize(self):
        ...
        for crop in self.crops_list:
            crop = Variable(crop.cuda(non_blocking=True), volatile=False)
            crop_list_gpu.append(crop)
        ...
        with torch.no_grad():
            ...
            for param_q, param_k in zip(self.net.student.module.parameters(), self.net.teacher.module.parameters()):
            ...
            for weight_names in weight_list:
                ...
                if namesplits[3][:3] <= best_loss_str:
                    ...
                    if is_Tbackbone_only:
                        torch.save({'model_state_dict': self.net.module.teacher.state_dict()}, path)
                    else:
