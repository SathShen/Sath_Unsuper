...
base_cfg.GLOBAL_RANK = 0
base_cfg.LOCAL_RANK = 0
...
base_cfg.DATA.BATCH_SIZE_PER_GPU = 4
...
base_cfg.LR_SCHEDULER.LEARNING_RATE = 4e-3
base_cfg.LR_SCHEDULER.FINAL_VALUE = 1e-6
base_cfg.LR_SCHEDULER.WARMUP_EPOCHS = 10
...
base_cfg.TT_SCHEDULER.TEACHER_TEMP = 0.07
base_cfg.TT_SCHEDULER.FINAL_VALUE = 0.07
base_cfg.TT_SCHEDULER.WARMUP_EPOCHS = 30
base_cfg.TT_SCHEDULER.WARMUP_VALUE = 0.04
...
def get_net_type(netname):
    ...
    if netname == 'dlinknet34' or netname == 'dlinknet50' or netname == 'dlinknet101':
        return 'dlinknet'
    if netname == 'dinov1' or netname == 'dinov2':
        return 'dino'
...
def update_config(config, args):
    ...
    if _check_args('cfg_path'):
        if args.cfg_path != 'None':
            assert os.path.exists(args.cfg_path), "配置文件路径不存在，退出程序！"
    ...
    if _check_args('pretrain_path'):
        assert os.path.exists(args.pretrain_path), "预训练模型路径不存在，退出程序！"
    ...
    if _check_args('output_path'):
        ...
        assert os.path.exists(config.OUTPUT_PATH), "输出路径不存在，退出程序！"
    if _check_args('seed'):
    ...
    if _check_args('train_data_path'):
        assert os.path.exists(args.train_data_path), "数据路径不存在，退出程序！"
        config.DATA.TRAIN_DATA_PATH = args.train_data_path
    if _check_args('valid_data_path'):
        assert os.path.exists(args.valid_data_path), "数据路径不存在，退出程序！"
        config.DATA.VALID_DATA_PATH = args.valid_data_path
    if _check_args('test_data_path'):
        assert os.path.exists(args.test_data_path), "数据路径不存在，退出程序！"
