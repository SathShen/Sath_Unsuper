...
base_cfg.LOSS.GAMMA = 2.
    # optimizer setting
    if _check_args('optimizer_name'):
        config.OPTIMIZER.NAME = args.optimizer_name.lower()
    if _check_args('optimizer_weight_decay'):
        config.OPTIMIZER.WEIGHT_DECAY = args.optimizer_weight_decay
    if _check_args('optimizer_eps'):
