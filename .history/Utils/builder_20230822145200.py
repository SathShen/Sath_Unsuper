...
class CosineScheduler(object):
    def __init__(self, start_value, end_value, total_iters, warmup_iters=0, start_warmup_value=0, freeze_iters=0, is_restart=False, T_0=5, T_mult=2):
        super().__init__()
        """
        ...
        if is_restart:
            ...
            schedule = end_value + 0.5 * (start_value - end_value) * (1 + np.cos(np.pi * iters / T_is))
        else:
            iters = np.arange(total_iters - warmup_iters - freeze_iters)
            schedule = end_value + 0.5 * (start_value - end_value) * (1 + np.cos(np.pi * iters / len(iters)))
    ...
    def get_restart_iters(self, num_iters, T_0, T_mult):
        ...
        while iter_accu < num_iters:
            T_i = T_mult ** num_cycles * T_0      # 2 ^ 0 * 5 = 5, 2 ^ 1 * 5 = 10
            iter_accu += T_i
...
def test_CosineScheduler():
    ...
    T_0 = 5
    T_mult = 2
    scheduler = CosineScheduler(max_value, min_value, total_iters, warmup_iters, start_warmup_value, freeze_iters, is_restart, T_0, T_mult)
