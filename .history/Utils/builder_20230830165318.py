...
def build_schedulers(cfg, epoch_length):
    lr = dict(
        base_value=cfg.LR_SCHEDULER.LEARNING_RATE,
    ...
    wd = dict(
        base_value=cfg.WD_SCHEDULER.WEIGHT_DECAY,
        final_value=cfg.WD_SCHEDULER.FINAL_VALUE,
        total_iters=cfg.NUM_EPOCHS * epoch_length,
    )
    momentum = dict(
        base_value=cfg.TM_SCHEDULER.TEACHER_MOMENTUM,
        final_value=cfg.TM_SCHEDULER.FINAL_VALUE,
        total_iters=cfg.NUM_EPOCHS * epoch_length,
    )
    teacher_temp = dict(
        base_value=cfg.TT_SCHEDULER.TEACHER_TEMP,
    ...
    last_layer_lr_scheduler.schedule[:cfg.optim["freeze_last_layer_epochs"] * epoch_length] = 0  # mimicking the original schedules
