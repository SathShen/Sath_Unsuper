...
def update_config(config, args):
    ...
    if _check_args('num_nodes'):
        config.NUM_NODES = args.num_nodes
    if _check_args('num_gpus_per_node'):
        config.NUM_GPUS_PER_NODE = args.num_gpus_per_node
    if _check_args('world_size'):
        config.WORLD_SIZE = args.world_size
    else:
        config.WORLD_SIZE = config.NUM_NODES * config.NUM_GPUS_PER_NODE
    ...
    if _check_args('is_pin_memory'):
        config.DATA.PIN_MEMORY = bool_flag(args.is_pin_memory)
