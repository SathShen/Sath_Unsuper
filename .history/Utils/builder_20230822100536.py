...
class CosineScheduler(object):
    def __init__(self, max_value, min_value, total_iters, warmup_iters=0, start_warmup_value=0, freeze_iters=0, is_restart=False, T_0=5, T_mult=2):
        ...
        if not is_restart:
            ...
            schedule = min_value + 0.5 * (max_value - min_value) * (1 + np.cos(np.pi * iters / len(iters)))  # FIXME warmup?
        iters = np.arange(total_iters - warmup_iters - freeze_iters)
        schedule = min_value + 0.5 * (max_value - min_value) * (1 + np.cos(np.pi * iters / len(iters)))  # FIXME warmup?
...
class CosineScheduler(object):
    def __init__(self, max_value, min_value, total_iters, T_0=0, T_mult=0, freeze_iters=0):
        super().__init__()
        ...
        iters = np.arange(total_iters - warmup_iters - freeze_iters)
        schedule = final_value + 0.5 * (base_value - final_value) * (1 + np.cos(np.pi * iters / len(iters)))  # FIXME warmup?
        self.schedule = np.concatenate((freeze_schedule, warmup_schedule, schedule))
