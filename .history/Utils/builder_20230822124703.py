...
class CosineScheduler(object):
    def __init__(self, max_value, min_value, total_iters, warmup_iters=0, start_warmup_value=0, freeze_iters=0, is_restart=False, T_0=5, T_mult=2):
        ...
        if is_restart:
            ...
            iters = self.get_restart_iters(num_iters, T_0, T_mult)
            schedule = min_value + 0.5 * (max_value - min_value) * (1 + np.cos(np.pi * iters / T_i))
        else:
    ...
    def get_restart_iters(num_iters, T_0, T_mult):
        iter_accu = 0
        num_cycles = 0
        sub_iters_tuple = ()
        T_i_list = []
        while iter_accu < num_iters:
            ...
            if iter_accu >= num_iters:
                num_T = iter_accu - num_iters - 

            T_i_list.extend([T_i] * num_T)          # [5, 5, 5, 5, 5, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, ]
            
            num_cycles += 1
            sub_iter = np.arange(T_i)
