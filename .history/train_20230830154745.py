...
def train(cfg, frame, train_dataset):
    ...
    train_data_loader = torch.utils.data.DataLoader(train_dataset, batch_size=cfg.DATA.BATCH_SIZE,
...
def get_parserargs():
    ...
    parser.add_argument('--lrs_warmup_epoch', '-lrswe', metavar='LRSWI', type=int, help='warmup epochs of cosinewarm scheduler')
    parser.add_argument('--lrs_warmup_value', '-lrswv', metavar='LRSSWV', type=float, help='start warmup value of cosinewarm scheduler')
    parser.add_argument('--lrs_freeze_epoch', '-lrsfe', metavar='LRSFI', default=0, type=int, help='freeze epochs of cosinewarm scheduler')
    ...
    parser.add_argument('--tts_warmup_epoch', '-ttswi', metavar='TTSWI', type=int, help='warmup epochs of cosinewarm scheduler')
...
def init_args(args):
    ...
    args.lrs_warmup_epoch = 10
    args.lrs_warmup_value = 0
    args.lrs_freeze_epoch = 0
    ...
    args.wds_warmup_epoch = 0
    args.wds_warmup_value = 0
    args.wds_freeze_epoch = 0
    ...
    args.tms_warmup_iters = 0
    args.tms_warmup_value = 0
    args.tms_freeze_iters = 0
    ...
    args.tts_warmup_iters = 30
    args.tts_warmup_value = 0
    args.tts_freeze_iters = 0
